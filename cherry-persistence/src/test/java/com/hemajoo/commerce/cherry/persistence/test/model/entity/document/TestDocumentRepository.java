...
class TestDocumentRepository extends BaseDatabaseUnitTest
{
    ...
    final void shouldCreateDocumentWithoutContentInDatabase()
    {
        ...
        Assertions.assertNotNull(documentRepository.findById(entity.getId()));
    }

    @Test
    @DisplayName("Create a document in the database with associated content in the content store")
    final void shouldCreateDocumentWithContentInDatabase() throws FileException, FileNotFoundException
    {
        // Generate a random document.
        DocumentEntity entity = DocumentRandomizer.generatePersistent(false);
        ...
        Assertions.assertNotNull(documentRepository.findById(entity.getId()));
        Assertions.assertNotNull(contentRepository.getContent(entity)); // TODO DocumentEntity should provide a getContent() service!
    }
}
